...
@section('contenido')
    ...
    <div class="container-fluid px-4">
        <header class="page-header page-header-dark pb-10"
            style="background-image: url({{ asset(Storage::disk('empresas')->url(session('empresa')->emitir_reconocimiento ?? '')) }}); background-size: cover; background-repeat: no-repeat;">
        ...
        <form method="POST" action="{{ route('respuesta.store') }}" role="form">
            ...
            <input type="hidden" name="encuestas_id"
                value="{{ $encuestas == null || count($encuestas) ? $encuestas[0]->encuestas_id : null }}">
                    ...
                    <div style="{{ $item->style }}" class="card text-white h-100">
                        <div class="card-body">
                            <div class="d-flex justify-content-between align-items-center">
                                ...
                                <img src="{{ Storage::disk('empresas')->url($item->imagen) }}" width="100px"
                                    height="100px" />
                            </div>
                        </div>
                        <div class="card-footer d-flex align-items-center justify-content-between small">
                            <input type="hidden" name="puntos[{{ $item->opciones_id }}]"
                                value="{{ $item->o_puntos }},{{ $item->descripcion }}">
                            <input name="opciones[]" type="checkbox" value={{ $item->opciones_id }}
                                {{ in_array($item->opciones_id, old('opciones') !== null ? old('opciones') : []) ? 'checked' : '' }}>
                            <i class="size-18 text-muted" data-feather="info" data-bs-toggle="tooltip"
                                data-bs-placement="top" title="{{ $item->detalle }}"></i>
                        </div>
                    </div>
                </div>
            ...
            <div class="form-group">
                ...
                <select name="user_id_reconocido" class="form-control" id="user_id_reconocido">
                    ...
                    @foreach ($users as $data)
                        <option value="{{ $data->id }}"{{ old('user_id_reconocido') == $data->id ? 'selected' : '' }}>
                ...
                <select name="grupal_id_reconocido[]" class="form-control" id="grupal_id_reconocido" multiple>
                    ...
                    @foreach ($grupal as $data)
                        <option value="{{ $data->id }}"
                            {{ old('grupal_id_reconocido') == $data->id ? 'selected' : '' }}> {{ $data->useryjefe }}
                        </option>
                    @endforeach
                </select>
            </div>
